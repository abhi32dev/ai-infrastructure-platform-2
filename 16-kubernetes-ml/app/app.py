...
async def health():
    ...
    # A Kubernetes httpGet readiness/liveness probe only inspects the HTTP
    # status code — it never parses the response body. The original
    # project-09 version of this endpoint always returned 200 with
    # "healthy": false/true in the body, which is correct for a caller
    # that reads JSON but is silently invisible to a K8s probe. Returning
    # 503 when unhealthy is what actually makes the readiness probe (and
    # therefore Service endpoint removal) work.
